...
> 拔掉电源时，短期记忆消失，  
> 但长期记忆依然存在！"*
> ![image-20250916154754245](C:\Users\7726\Desktop\Inbox\Crash\note-crash\image\image-20250916154754245.png)

---

### **基本概念：内存 vs 存储**

![image-20250916154854613](C:\Users\7726\Desktop\Inbox\Crash\note-crash\image\image-20250916154854613.png)
...
![image-20250916155119314](C:\Users\7726\Desktop\Inbox\Crash\note-crash\image\image-20250916155119314.png)

| 技术                                    | 工作原理           | 特点                  | 局限                     |
| --------------------------------------- | ------------------ | --------------------- | ------------------------ |
| **延迟线存储器**<br>(Delay Line Memory) | 液体管中压力波传输 | 1944年发明，ENIAC使用 | 顺序访问，密度低，易混   |
...
#### 1. **存储金字塔**
   ...
   pyramid
       title 存储层次结构
       "寄存器（最快，最小）" --> "CPU缓存（L1/L2/L3）"
       "CPU缓存（L1/L2/L3）" --> "主内存（RAM）"
       "主内存（RAM）" --> "固态存储（SSD）"
       "固态存储（SSD）" --> "机械硬盘（HDD）"
       "机械硬盘（HDD）" --> "磁带/云存储（最慢，最大）"
   ```
